...
import { Program, AttributeData, UniformData, TextureData, Context, ElementsBundle, Index, Bundle } from '../../../engine1/engine.core';
...
class SplineRenderer {
    ...
    constructor(data: FeatureCollection<Point, GridPointProps>) {
        ...
        let nrCols = Math.max(... features.map(f => f.properties.col)) + 1;
        nrCols = nextPowerOf(nrCols, 2);
        let nrRows = Math.max(... features.map(f => f.properties.row)) + 1;
        nrRows = nextPowerOf(nrRows, 2);
        ...
        this.context = new Context(this.canvas.getContext('webgl'), false);
