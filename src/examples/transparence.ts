...
import { boxE, boxA } from '../engine/engine.shapes';
import { flattenRecursive, transposeMatrix, matrixMultiplyList, rotateXMatrix, translateMatrix, scaleMatrix, matrixMultiply, rotateYMatrix, projectionMatrix, rotateZMatrix } from '../engine/math';
...
const boxInitialTransform = matrixMultiplyList([translateMatrix(0, 0, -2.2), rotateZMatrix(0), rotateYMatrix(0), scaleMatrix(0.2, 0.2, 0.2)]);
...
const glassInitialTransform = matrixMultiplyList([translateMatrix(0.1, 0.1, -1), rotateZMatrix(0.2), rotateYMatrix(0.3), scaleMatrix(0.2, 0.2, 0.2)]);
const glassShader = new Shader(
    ...
    [
        ...
        new Uniform(gl, simple3DProgram, 'u_model', 'mat4', flattenRecursive(transposeMatrix(glassInitialTransform))),
        new Uniform(gl, simple3DProgram, 'u_projection', 'mat4', flattenRecursive(transposeMatrix(projectionMatrix(Math.PI / 4, 1, 0.01, 50)))),
...
renderLoop(60, (deltaT: number) => {
    ...
    const newBoxModelMatrix = matrixMultiplyList([translateMatrix(0, 0, -2.6), rotateZMatrix(boxRotation), rotateYMatrix(boxRotation), scaleMatrix(0.2, 0.2, 0.2)]);
    boxShader.updateUniformData(gl, 'u_model', flattenRecursive(transposeMatrix(newBoxModelMatrix)));
    boxShader.bind(gl);
    boxShader.render(gl);

    const glassRotation = (0.01 * t) % (2 * Math.PI);
    const newGlassModelMatrix = matrixMultiplyList([translateMatrix(0.1, 0.1, -1), rotateZMatrix(glassRotation), rotateYMatrix(0.3), scaleMatrix(0.2, 0.2, 0.2)]);
    glassShader.updateUniformData(gl, 'u_model', flattenRecursive(transposeMatrix(newGlassModelMatrix)));
    glassShader.bind(gl);
